...
class LocationMap extends Component {
  ...
  componentDidMount = () => {
    const { location, photographOnboard } = this.context.state;
    if(photographOnboard && photographOnboard.latitude) {
      const obj =  {
        latitude: photographOnboard.latitude,
        longitude: photographOnboard.longitude,
        latitudeDelta: LATITUDE_DELTA,
        longitudeDelta: LONGITUDE_DELTA,
      }
      const defaultMarker = {
        coordinate: { longitude: photographOnboard.longitude, latitude: photographOnboard.latitude },
        key: id++,
        color: randomColor()
      }
  ...
  getAddressDetails = (res) => {
    ...
    if(state.photographOnboard) {
      const locationObj = { longitude: geometryloc.lng, latitude: geometryloc.lat, state: stateObj.long_name, 
        ...
        address: state.photographOnboard.address ? state.photographOnboard.address : formatted_address, 
        district: localityObj.long_name}

      const obj = { ...state.photographOnboard, ...locationObj }
      set({ photographOnboard: obj })
      this.props.navigation.navigate('PhotographStack', { screen: 'PhotographEquipments'})
    } 
  }
  renderMapView = () => {
    const { region } = this.state;
            ...
            <ScrollView>
              <View style={container}>
                  <View style={{flex: 1, height: '100%'}}>
                          ...
                          <View style={[button]}>
                              <CustomButton buttonText="Next" buttonStyle={{ elevation: 2}} onPress={this.getGeolocation} />
                          </View>
