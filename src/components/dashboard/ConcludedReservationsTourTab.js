...
class ConcludedReservationsTourTab extends Component {
    constructor(props) {
      super(props);
    ...
    renderItem = ({item}) => {
      ...
      const imgUrl = item.propertyImage ? {uri: item.propertyImage} : require('../../assets/images/no_experience.png')
      const reserve = `${item.reservations} reservation(s)`
      const payment = item.payment ? formatAmount(item.payment) : 0.00
      return (
          <View style={rowContainer}>
              {/* <ReservationRow title={title} img={imgUrl} {...this.props} payment={payment}
    ...
    renderEmptyContainer = () => {
        ...
        if (state.concludedReservations.length === 0 && !state.loadingConReservations) {
            ...
            <View style={{ flex: 1, paddingVertical: 60}}>
                ...
                <MyText style={[textBold, textCenter, textH4Style, textOrange]}>No Concluded Reservations Yet</MyText>
            </View>
            )
        }
    }
  ...
  export default ConcludedReservationsTourTab;
