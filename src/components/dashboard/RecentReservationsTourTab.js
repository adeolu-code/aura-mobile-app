...
import ReservationRow from './ReservationTourRow';

import colors from '../../colors';
import { Icon } from 'native-base';

class RecentReservationsTourTab extends Component {
    constructor(props) {
      super(props);
    ...
    renderItem = ({item}) => {
      ...
      const imgUrl = item.experienceImage ? {uri: item.experienceImage} : require('../../assets/images/no_experience.png')
      const reserve = `${item.reservations} reservation(s)`
      const payment = item.payment ? formatAmount(item.payment) : 0.00
        return (
            <View style={rowContainer}>
                {/* <ReservationRow  propertyType={item.propertyType}  
                 onPress={this.linkToReserve.bind(this, item)} /> */}
                <ReservationRow img={imgUrl} title={title} {...this.props} reserve={reserve} payment={payment} />
            </View>
        )
       }
    ...
    renderEmptyContainer = () => {
        ...
        if(state.reservations.length === 0 && !state.loadingReservations) {
            ...
            <View style={{ flex: 1, paddingVertical: 60}}>
                ...
                <MyText style={[textBold, textCenter, textH4Style, textOrange]}>No Recent Reservations Yet</MyText>
            </View>
            )
        }
    }
  ...
  export default RecentReservationsTourTab;
