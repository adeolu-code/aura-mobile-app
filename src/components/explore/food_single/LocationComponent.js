...
class LocationComponent extends Component {
  ...
  getGeolocation = async (location) => {
      ...
      const res = await GetRequest('https://maps.googleapis.com/maps/', `api/geocode/json?address=${location.street},${location.city},${location.country}&key=${GOOGLE_API_KEY}`)
      this.getAddressDetails(res.results[0], location)
  }

  getAddressDetails = (res, location) => {
      const geometryloc = res.geometry.location
      const addressComponents = res.address_components
      const locationObj = { longitude: geometryloc.lng, latitude: geometryloc.lat, city: location.city, state: location.state, country: location.country }
      this.createMarkers(locationObj)
      // return locationObj;
  }
