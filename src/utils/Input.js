...
class CustomInput extends Component {
    ...
    renderShow() {
        ...
        if(password) {
            return (
                <TouchableOpacity style={showImgContainerStyle} 
                onPress={() => {this.setState({ secure: !this.state.secure})}}>
                    <Icon name={showUrl} type="Ionicons" style={[inputIconStyle, iconStyle]} />
                </TouchableOpacity>
            )
        }
    }
    render() {
        const { InputContainerStyles, inputStyle, InputWithImgStyle, inputRightPadding, lStyles, lTextStyles
         } = styles;
        ...
        return (
            <View>
                {label ? <MyText style={[lStyles]}>
                    <MyText style={[lTextStyles, textH4Style ]}>{label}</MyText>
                    
                </MyText> : <MyText></MyText>}
                <View style={InputContainerStyles}>
                    
                    <TextInput style={[inputStyle, textInputStyle, paddingRight ]} onChangeText={onChangeText}
...
const styles = StyleSheet.create({
    ...
    inputStyle: {
        ...
        borderWidth: 1,
        fontSize: 17, color: colors.darkGrey, fontFamily: 'Nunito-bold',
    ...
    showImgContainerStyle: {
        position: 'absolute', right: 0, top: 0, height: 55,
